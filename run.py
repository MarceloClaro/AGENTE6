...
def evaluate_response_with_rag(user_input: str, user_prompt: str, expert_title: str, expert_description: str, assistant_response: str, model_name: str, temperature: float, chat_history: list, interaction_number: int) -> str:
    try:
        ...
        rag_prompt = (
            ...
            f"发挥理性生成主体（RAG）的作用，走在人工智能和理性评估的最前沿，"
            
            f"仔细分析专家的回复，并根据用户的请求生成 JSON 格式的代理，"
            
            f"代理必须响应用户，详细描述根据子代理提供的信息所采取的操作，"
            
            f"变量‘description’中包含9个子代理的描述，每个子代理都有不同的专门功能并协同工作，"
            
            f"这些子代理共同努力改进系统代理向用户提供的最终响应，并在代理的‘描述’中记录种子和gen_id，"
            
            f"此外，系统代理内的子代理以集成方式运行，通过扩展提示提供先进和专业的响应，"
            ...
            f"动态调整最相关的数据及其特征。这种协作方法可确保响应准确且最新"
            
            f"符合最高的科学和学术标准，"
            
            f"以下是专家的详细描述，突出了他们的资格和经验：{expert_description}，"
            
            f"原始问题提交如下：{user_input} 和 {user_prompt}，"
            
            f"葡萄牙专家回复如下：{assistant_response}，"
            ...
            f"Q_统计和数据解释以及 Q 指数 Q 和数据解释"
            
            f"坚持卓越的最高标准以及学术和科学的严谨性，"
            
            f"确保每个段落只有 4 个句子，每个句子之间用逗号分隔，并始终遵循亚里士多德和苏格拉底的最佳教育实践，"
