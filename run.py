...
API_KEYS = {
    ...
    "refine": ["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"],
    "evaluate": ["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"]
}

current_api_key_index = {"fetch": 0, "refine": 0, "evaluate": 0}

def get_api_key(action: str) -> str:
    return API_KEYS[action][current_api_key_index[action]]

def switch_api_key(action: str):
    current_api_key_index[action] = (current_api_key_index[action] + 1) % len(API_KEYS[action])
...
def log_api_usage(action: str, interaction_number: int, tokens_used: int, time_taken: float, user_input: str, user_prompt: str, response: str):
    entry = {
        'action': action,
        ...
        while True:
            try:
                ...
                if agent_selection == "Escolher um especialista...":
                    ...
                    first_period_index = phase_one_response.choices[0].message.content.find(".")
                    expert_title = phase_one_response.choices[0].message.content[:first_period_index].strip()
                    expert_description = phase_one_response.choices[0].message.content[first_period_index + 1:].strip()
                ...
                end_time = time.time()
                tokens_used = phase_two_response.usage.total_tokens
                time_taken = end_time - start_time
                log_api_usage('fetch', interaction_number, tokens_used, time_taken, user_input, user_prompt, phase_two_response.choices[0].message.content)

                return expert_title, phase_two_response.choices[0].message.content
    ...
    try:
        while True:
            try:
                ...
                end_time = time.time()
                tokens_used = refined_response.usage.total_tokens
                time_taken = end_time - start_time
                log_api_usage('refine', interaction_number, tokens_used, time_taken, user_input, user_prompt, refined_response.choices[0].message.content)
    ...
    try:
        while True:
            try:
                ...
                time_taken = end_time - start_time
                log_api_usage('evaluate', interaction_number, tokens_used, time_taken, user_input, user_prompt, rag_response.choices[0].message.content)
